...
- **Survey handoff.** Run `pnpm verify:handoff` (add `--live` with a dev server running to drive a real interview and delete its rows afterward). It asserts the closing question still trips the end matcher, that `QUALTRICS_FOLLOWUP_URL` is set and reachable, and that the follow-up survey's flow declares `chat_id`, `participant_seq` and `completion_code` above the first block. Both failure modes are otherwise silent: an unset URL yields no link at all, and undeclared embedded data is dropped by Qualtrics without an error, leaving human responses that cannot be joined to a transcript. As of 2026-07-31 both are outstanding: `QUALTRICS_FOLLOWUP_URL` is unset in production, and the human survey (`SV_204E1t0lUA39DJY`, "Mlt-Survey") has no Embedded Data element at all, unlike the agent survey.
